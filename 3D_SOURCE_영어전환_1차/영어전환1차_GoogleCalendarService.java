...
public class GoogleCalendarService {
	...
	*/
    public boolean init() {
        ...
        try {
            ...
            if (tokenFile.exists()) {
                loadToken(tokenFile);
                System.out.println("[GCalendar] 기존 토큰 로드 Done");
			}
	...
	*/
    public boolean credentialsExist() {
        ...
        System.out.println("[GCalendar] CREDENTIALS_FILE 탐색 Path: " + dest.getAbsolutePath());
        ...
        if (tempDir.exists() && tempDir.isDirectory()) {
            ...
            if (src.exists()) {
                try {
                    ...
                    System.out.println("[GCalendar] CREDENTIALS_FILE 자동 복사 Done: "
					+ src.getAbsolutePath() + " -> " + dest.getAbsolutePath());
                    return true;
					} catch (Exception e) {
                    System.out.println("[GCalendar] CREDENTIALS_FILE 복사 Failed: " + e.getMessage());
				}
				} else {
                System.out.println("[GCalendar] C:\\temp\\CREDENTIALS_FILE 없음");
			}
			} else {
            System.out.println("[GCalendar] C:\\temp Folder 없음");
		}
        System.out.println("[GCalendar] CREDENTIALS_FILE 없음 - Calendar initialization 생략");
        return false;
	}
    /**
	...
	*/
    public String logout() {
        ...
        if (tokenFile.exists()) {
            ...
            tokenFile.delete();
            System.out.println("[GCalendar] 로그아웃 Done - 토큰 Delete: " + path);
            return path;
		}
        System.out.println("[GCalendar] 로그아웃 Done - 토큰 File not found");
        return null;
	}
    // ── 공개 API: 정 조회 ───────────────────────────────────────
    /** Today schedule 조회 */
    ...
    private List<CalendarEvent> getEvents(ZonedDateTime start, ZonedDateTime end) {
        ...
        if (!initialized) {
            System.out.println("[GCalendar] initialization되지 않음");
            return result;
		}
        try {
            ensureValidToken();
	...
	*/
        ...
        try {
            ...
            for (Object obj : items) {
                try {
                    ...
                    System.out.println("[GCalendar] 이벤트 File싱 오류: " + e.getMessage());
				}
			}
			} catch (Exception e) {
            System.out.println("[GCalendar] events JSON File싱 오류: " + e.getMessage());
		}
        return list;
	}
    ...
    private boolean doBrowserAuth() throws Exception {
        ...
        if (code == null || code.isEmpty()) {
            System.out.println("[GCalendar] 인증 코드 수신 Failed");
            return false;
		}
        // code → token 교환
        exchangeToken(code);
    ...
    private void exchangeToken(String code) throws Exception {
        ...
        saveToken();
        System.out.println("[GCalendar] 토큰 교환 Done");
	}
    ...
    private synchronized void refreshAccessToken() throws Exception {
        ...
        parseAndStoreToken(json);
        saveToken();
        System.out.println("[GCalendar] 토큰 갱신 Done");
	}
    ...
    private void parseAndStoreToken(String json) {
        try {
            ...
            System.out.println("[GCalendar] 토큰 File싱 오류: " + e.getMessage());
		}
	}
    private synchronized void saveToken() {
        try {
            ...
            System.out.println("[GCalendar] 토큰 Save Failed: " + e.getMessage());
		}
	}
    private synchronized void loadToken(File f) {
        ...
        try {
            ...
            System.out.println("[GCalendar] 토큰 로드 Failed: " + e.getMessage());
		}
	}
    // ── 내부: HTTP 유틸 ───────────────────────────────────────────
