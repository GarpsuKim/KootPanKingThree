...
public class KootPanKingThreeApp {
	...
	private javafx.scene.control.Menu buildPhoneCamMenu(
        ...
        camStart.setOnAction(e -> {
            ...
            btnFfmpeg.setOnAction(ev -> {
                ...
                if (chFf != null && chFf.isFile()) {
                    ...
                    System.out.println("[FFmpeg] 지정 Done: " + AppContext.getFfmpegPath());
				}
			});
        ...
        camInstall.setOnAction(e -> {
            try {
                ...
                System.out.println("[CameraInstall] 브라우저 열기 Failed: " + ex.getMessage());
			}
		});
        ...
        camGuide.setOnAction(e -> {
            ...
            new Thread(() -> {
                try {
                    ...
                    System.out.println("[CameraGuide] 브라우저 열기 Failed: " + ex.getMessage());
				}
            ...
            if (tgOk) {
                new Thread(() -> {
                    ...
                    try { tg.sendTelegram( GUIDE_MSG);
                        System.out.println("[CameraGuide] 텔레그램 전송 Done");
						} catch (Exception ex) {
                        System.out.println("[CameraGuide] 텔레그램 전송 Failed: " + ex.getMessage());
					}
            ...
            if (emailOk) {
                new Thread(() -> {
                    try { gmail.sendOneSmtp(gmail.lastTo, GUIDE_SUBJECT, GUIDE_MSG);
						System.out.println("[CameraGuide] 이Sending email Done");
						} catch (Exception ex) {
                        System.out.println("[CameraGuide] 이Sending email Failed: " + ex.getMessage());
	...
	private javafx.scene.control.Menu buildCctvMenu(
        ...
        cctvGuide.setOnAction(e -> {
            try {
                ...
                System.out.println("[ItsCctv] 브라우저 열기 Failed: " + ex.getMessage());
			}
		});
        cctv.setOnShowing(e -> {
            ItsCctvManager mgr = getItsCctv();
	...
	*/
    ...
    void startCamera(String streamUrl) {
        ...
        System.out.println("[Camera] 연결 Start: " + streamUrl);
	}
	...
	*/
    void stopItsCctv() {
        ...
        System.out.println("[ItsCctv] 중지 Done → 이전 배경 복원");
	}
    // ══════════════════════════════════════════════════════════════════
	...
	*/
    void startLocalMp4(File file) {
        ...
        saveConfig();
        System.out.println("[KPK] 로컬 MP4 재생 Start: " + file.getName());
	}
	...
	*/
    ...
    /** 현재 프레임을 APP_DIR 에 SaveOther고 결과 다이얼로그 Show */
    void captureCamera(javafx.stage.Stage owner) {
        ...
        if (saved != null) {
            System.out.println("[Camera] 캡처 Save: " + saved);
            ...
            javafx.application.Platform.runLater(() -> {
                ...
                alert.setHeaderText(null);
                alert.setContentText("📸 Saved: img/" + fileName);
                alert.show();
                // 2초 후 자동 닫힘
	...
	*/
    void startVideoRecording(File outputFile) {
        ...
        if (!ffOk) {
            ...
            System.out.println("[VideoRec] FFmpeg 불가 → 5초 Image Save으로 자동 전환");
	...
	*/
    private String findFfmpeg() {
        ...
        if (ffmpegPath == null || ffmpegPath.trim().isEmpty()) {
            System.out.println("[FFmpeg] ffmpeg.path 미Settings");
            return null;
		}
	...
	*/
    ...
    private void packFramesToZip(File frameDir, File zipFile, int frameCount) {
        try (java.util.zip.ZipOutputStream zos =
            ...
            System.out.println("[VideoRec] ZIP 폴백 Save: " + zipFile.getAbsolutePath());
			} catch (Exception ex) {
	...
	*/
    void stopImageSequenceRecording() {
        ...
        System.out.println("[ImgSeq] 중지 — Save File 수: " + saved);
	}
    ...
    private void showVideoResult(boolean mp4Ok, File outputFile,
        ...
        btnOpen.setOnAction(ev -> {
            try {
                ...
                System.out.println("[VideoRec] Folder 열기 Failed: " + ex.getMessage());
			}
		});
        javafx.scene.layout.HBox btnRow = new javafx.scene.layout.HBox(10, btnOpen, btnOk);
        btnRow.setAlignment(javafx.geometry.Pos.CENTER_RIGHT);
    ...
    private void openConfigFile() {
		/*
            ...
            System.err.println("Settings File 열기 Failed: " + e.getMessage());
			}
		*/
	}
    ...
    private void showAboutDialog() {
        ...
        link.setOnAction(ev -> {
            try {
                ...
                System.out.println("[About] Links 열기 Failed: " + ex.getMessage());
			}
		});
        // ── OK 버튼 ───────────────────────────────────────────
        javafx.scene.control.Button okBtn = new javafx.scene.control.Button("OK");
