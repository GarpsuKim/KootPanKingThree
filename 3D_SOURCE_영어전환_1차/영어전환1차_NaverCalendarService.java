...
public class NaverCalendarService {
    ...
    private final Set<String> processedEventKeys = Collections.newSetFromMap(
		...
		public NaverCalendarService() {
			...
			System.out.println("[NaverCal] ical4j 타임존 Settings Done");
		}
		...
		public void setCredentials(String id, String password) {
			...
			System.out.println("[NaverCal] 자격 증명 Settings: " + naverId);
		}
		...
		public boolean init() {
			if (naverId.isEmpty() || naverPassword.isEmpty()) {
				System.out.println("[NaverCal ERROR] initialization Failed: 자격 증명이 없음");
				return false;
			}
			
			System.out.println("[NaverCal] 네이버 캘린더 initialization 시작...");
			...
			try {
				...
				if (principalUrl == null || principalUrl.isEmpty()) {
					System.out.println("[NaverCal ERROR] current-user-principal 추출 Failed");
					return false;
				}
				...
				if (homeUrl == null || homeUrl.isEmpty()) {
					System.out.println("[NaverCal ERROR] calendar-home-set 추출 Failed");
					return false;
				}
				
				calendarHomeUrl = homeUrl.startsWith("http") ? homeUrl : CALDAV_BASE + homeUrl;
				initialized = true;
				System.out.println("[NaverCal] initialization 성공! 캘린더 홈 URL: " + calendarHomeUrl);
				return true;
				
				} catch (Exception e) {
				System.out.println("[NaverCal ERROR] initialization 중 예외 발생: " + e.getMessage());
				e.printStackTrace();
				return false;
			}
		}
		...
		public List<CalendarEvent> getToday() {
			...
			System.out.println("[NaverCal] 오늘 정 조회 중...");
		...
		public List<CalendarEvent> getNextDays(int days) {
			...
			System.out.println("[NaverCal] " + days + " 후까지 정 조회 중...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getPastDays(int days) {
			...
			System.out.println("[NaverCal] 지난 " + days + " 정 조회 중...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getThisWeek() {
			...
			System.out.println("[NaverCal] 이번주 정 조회 중...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getThisMonth() {
			...
			System.out.println("[NaverCal] 이번달 정 조회 중...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getNextMonth() {
			...
			ZonedDateTime start = firstDay.atStartOfDay(ZoneId.systemDefault());
			ZonedDateTime end   = lastDay.atTime(23, 59, 59).atZone(ZoneId.systemDefault());
			System.out.println("[NaverCal] 다음달 정 조회 중...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getUpcomingAlarms(int withinMinutes) {
			...
			System.out.println("[NaverCal] " + withinMinutes + "분 내 정 조회 중...");
		...
		private List<CalendarEvent> getEvents(ZonedDateTime start, ZonedDateTime end) {
			...
			if (!initialized) {
				System.out.println("[NaverCal WARNING] 서비스가 initialization되지 않음");
				return result;
			}
			...
			try {
					...
					try {
						...
						for (String icsText : icsTexts) {
							...
							try {
								...
								System.out.println("[NaverCal ERROR] ICS File싱 오류: " + e.getMessage());
							}
						}
					} catch (Exception e) {
				...
				System.out.println("[NaverCal] 총 " + result.size() + "개 정 찾음");
				...
				for (Object comp : cal.getComponents(Component.VEVENT)) {
					...
					try {
						...
						if (msg.contains("Unsupported unit") || msg.contains("Unable to obtain")) {
							System.out.println("[NaverCal DEBUG] 부 정 File싱 Failed (무시): " + msg);
						} else {
		...
		private List<String> fetchIcsTexts(String calUrl,
			...
			try {
				...
				System.out.println("[NaverCal] multiget Failed → 개별 GET 폴백: " + e.getMessage());
			}
		...
		private String caldavRequest(String method, String urlStr,
			...
			*/
			...
			try (javax.net.ssl.SSLSocket sock =
				...
				if (statusCode == 401) {
					System.out.println("[NaverCal ERROR] 인증 Failed (401) - 아이디/Password 확인 필요");
					return null;
				}
		...
		public void cleanup() {
			...
			System.out.println("[NaverCal] 리소스 정리 Done");
		}
