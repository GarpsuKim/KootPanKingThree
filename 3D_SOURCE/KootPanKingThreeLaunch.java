...
public class KootPanKingThreeLaunch extends Application {
	...
	public static TOOLS.AliveStatusAgent aliveStatusAgent = new TOOLS.AliveStatusAgent();
    ...
    public void start(Stage primaryStage) {
		...
		mainWindow.onLockHooksOn = () -> {
			try {
				...
				System.out.println("[SuperLock] swallow 훅 종료 실패: " + e.getMessage());
			}
			// 일반 훅으로 복원
			setupMouseHook();
			setupKeyboardHook();
			System.out.println("[SuperLock] 일반 훅 복원 완료");
		};
	}  // ── end of start() ──────────────────────────────────
	private static void showStartupScheduleTextLater() {
		new Thread(() -> {
			...
			if (mainWindow == null) {
				System.out.println("mainWindow not initialized");
				return;
			}
			// if (startupScheduleText != null && !startupScheduleText.isEmpty()) {
			javafx.application.Platform.runLater(() -> {
				System.out.println("event notification dialog registered");
				firstFinalGmail();
				KakaoSetup();
	...
	private static void telegramSetup() {
		...
		tg = TelegramBot.getInstance(new TelegramBot.CommandHandler() {
			...
			@Override public void shutdownPC() {
				System.out.println("=======Telegram remote shutdown");
			...
			@Override public void superLock() {
				javafx.application.Platform.runLater(() -> {
					if (mainWindow != null) mainWindow.superLockScreen();
				});
			}
			@Override public void superUnlock() {
				// superUnlockScreen() 내부에서 isFxApplicationThread 체크 후 runLater 처리
				if (mainWindow != null) mainWindow.superUnlockScreen();
			}
		});
		...
		*/
		...
		new Thread(() -> {
            ...
            try {
				...
				System.out.println("[CalendarInit] event query Failed: " + e.getMessage());
				AppLogger.logException(e);
			}
		}, "CalendarInit").start();
	...
	private static void setupMouseHook() {
		try {
						...
						Platform.runLater(() -> {
							...
							if (app != null && app.clockController != null) {
								...
								if (clockStage != null) {
									...
									System.out.println("[MouseHook] clock to front");
								}
							}
			...
			System.out.println("[MouseHook] global mouse hook started");
		} catch (Exception e) {
			System.out.println("[MouseHook] hook initialization failed: " + e.getMessage());
		}
	}
	...
	private static void setupKeyboardHook() {
		try {
						...
						Platform.runLater(() -> {
							...
							if (app != null && app.clockController != null) {
								...
								if (clockStage != null) {
									...
									clockStage.requestFocus();
									clockStage.setAlwaysOnTop(false);
									System.out.println("[KeyboardHook] clock to front");
								}
							}
							// 메인창 토글
							if (mainWindow != null)
			...
			System.out.println("[KeyboardHook] global keyboard hook started");
		} catch (Exception e) {
			System.out.println("[KeyboardHook] hook initialization failed: " + e.getMessage());
		}
	}
