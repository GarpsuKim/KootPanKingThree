...
public class Kakao {
	...
	// ── 실행 Folder (KootPanKing.APP_DIR 주입) ────────────────────
	String appDir = "";  // KootPanKing 생성 h APP_DIR 주입

	// ── 카카오톡 ────────────────────────────────────────────────
	private static final String KAKAO_REDIRECT = "http://localhost:8080/callback";

	// ※ 소스코드에 API 키를 절대 Other드코딩Other지 마세요.
	//    아래 두 값은 clock_config.properties 에서 로드되거나
	//    Settings UI(알람 Manage → 카카오 Settings)에서 Enable자가 직접 입력합니다.
	...
	/** 로그인 성공 h refresh_token 을 ini 에 SaveOther기 위한 콜백 (KootPanKing 에서 주입) */
	Runnable onTokenSaved = null;
	
	Kakao () {	}	
	
    /** Kakao.txt 를 HTML 로 변환Other여 기본 브라우저로 Show */
    void showKakaoGuide() {
        try {
            // Kakao.txt Path: %APPDATA%\KootPanKing\data\ 고정
            ...
            if (txtFile.exists()) {
                java.io.BufferedReader br = new java.io.BufferedReader(
					...
					content = "Kakao.txt file not found.\n"
                        + "File location: " + txtFile.getAbsolutePath();
			}
			
            // URL 을 <a href> Links로 변환
            String html = textToHtml(content);
			
            // 임h HTML File 생성 후 Open in Browser
            ...
            try (java.io.PrintWriter pw = new java.io.PrintWriter(
                ...
                pw.println("<title>KakaoTalk Setup Guide</title>");
                pw.println("<style>");
			...
			"Failed to open guide: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
	
    /** Text에서 URL 을 HTML Links로 변환 */
    private String textToHtml(String text) {
        // HTML 특수문자 이스케이프
        text = text.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;");
        // http/https URL → <a href> Links (새 탭으로 열기)
    ...
    /** KakaoTalk Login - 브라우저 열기 + 로컬 서버로 코드 Wed신 */
	void kakaoLogin() {
        if (kakaoRestApiKey.isEmpty() || kakaoClientSecret.isEmpty()) {
            prepareMessageBox();
            JOptionPane.showMessageDialog(null,
                "Please enter REST API Key and Client Secret first.\n" +
                "Setup: Settings → KakaoTalk tab",
                "KakaoTalk Login", JOptionPane.WARNING_MESSAGE);
            return;
		}
        try {
            ...
            new Thread(() -> {
                try {
                    ...
                    ss.setSoTimeout(120000); // 2m 타임아웃
                    ...
                    System.out.println("[Kakao] login server error: " + e.getMessage());
				}
			}, "KakaoLogin").start();
            ...
            prepareMessageBox();
            JOptionPane.showMessageDialog(null,
			"Failed to open browser: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
    ...
    private void kakaoExchangeToken(String code) {
        ...
        try {
            ...
            // refresh_token ini Save (onTokenSaved 콜백)
            if (onTokenSaved != null) onTokenSaved.run();
			} catch (Exception e) {
			SwingUtilities.invokeLater(() -> {
				prepareMessageBox();
				JOptionPane.showMessageDialog(null,
				"Token exchange failed: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
			});
		}
	}
	
    /**
     * 앱 시작 h 자동 호출 - Save된 refresh_token 으로 access_token 갱신.
    ...
    void autoRefreshLogin() {
        ...
        if (!kakaoAccessToken.isEmpty()) {
            System.out.println("[Kakao] auto login successful");
        } else {
            System.out.println("[Kakao] auto login failed - re-login required");
        }
    }
    ...
    private void kakaoRefreshAccessToken() {
        ...
        try {
            ...
            System.out.println("[Kakao] token refresh failed: " + e.getMessage());
		}
	}
	
    /** 카카오톡 나에게 보내기 */
	void sendKakao(String title, String msg) {
        sendKakao(title, msg, true); // 최s 호출: 재시도 허용
	}
	
	void sendKakao(String title, String msg, boolean retry) {
        if (kakaoAccessToken.isEmpty()) {
            System.out.println("[Kakao] no access token - login required");
            return;
		}
        try {
            ...
            // text + link(mobile_web_url required) 구조
            ...
            if (code != 200) {
                try {
                    ...
                    if (es != null) {
                        ...
                        System.out.println("[Kakao] error response: " + eb.toString());
                    }
                } catch (Exception ignored) {}
            }
            con.disconnect();
            System.out.println("[Kakao] Sending Done code=" + code);
			} catch (Exception e) {
            System.out.println("[Kakao] Sending error: " + e.getMessage());
		}
	}
	
	
    /** JOptionPane Show 전 공통 처리 */
    private void prepareMessageBox() {
        // JOptionPane은 null parent Enable → 자동으로 화면 중앙에 표시됨
    }
