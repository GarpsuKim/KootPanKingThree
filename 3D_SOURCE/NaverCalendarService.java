...
/**
	* NaverCalendarService - 네이버 캘린더 CalDAV Integrations
	* 
...
public class NaverCalendarService {
    ...
    private final Set<String> processedEventKeys = Collections.newSetFromMap(
		...
		public NaverCalendarService() {
			...
			System.out.println("[NaverCal] ical4j timezone setup complete");
		}
		...
		public void setCredentials(String id, String password) {
			...
			System.out.println("[NaverCal] credentials Settings: " + naverId);
		}
		...
		public boolean init() {
			if (naverId.isEmpty() || naverPassword.isEmpty()) {
				System.out.println("[NaverCal ERROR] initialization Failed: credentials missing");
				return false;
			}
			
			System.out.println("[NaverCal] Naver Calendar initialization start...");
			
			try {
				// 1단계: current-user-principal
				System.out.println("[NaverCal] 1. PROPFIND / requesting...");
				String rootResp = caldavRequest("PROPFIND", CALDAV_BASE + "/",
				propfindPrincipalBody(), "application/xml", "0");
				if (rootResp == null) {
					System.out.println("[NaverCal ERROR] PROPFIND no root response");
					return false;
				}
				
				String principalUrl = extractHref(rootResp, "current-user-principal");
				if (principalUrl == null || principalUrl.isEmpty()) {
					System.out.println("[NaverCal ERROR] current-user-principal extract failed");
					return false;
				}
				...
				System.out.println("[NaverCal] 2. calendar-home-set requesting...");
				String propResp = caldavRequest("PROPFIND", absPrincipalUrl,
				propfindHomeSetBody(), "application/xml", "0");
				if (propResp == null) {
					System.out.println("[NaverCal ERROR] calendar-home-set PROPFIND Failed");
					return false;
				}
				
				String homeUrl = extractHref(propResp, "calendar-home-set");
				if (homeUrl == null || homeUrl.isEmpty()) {
					System.out.println("[NaverCal ERROR] calendar-home-set extract failed");
					return false;
				}
				
				calendarHomeUrl = homeUrl.startsWith("http") ? homeUrl : CALDAV_BASE + homeUrl;
				initialized = true;
				System.out.println("[NaverCal] initialization success! calendar home URL: " + calendarHomeUrl);
				return true;
				
				} catch (Exception e) {
				System.out.println("[NaverCal ERROR] initialization exception occurred during: " + e.getMessage());
				e.printStackTrace();
				return false;
			}
		}
		...
		public List<CalendarEvent> getToday() {
			...
			System.out.println("[NaverCal] Today event query in progress...");
		...
		public List<CalendarEvent> getNextDays(int days) {
			...
			System.out.println("[NaverCal] " + days + " days ahead event query in progress...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getPastDays(int days) {
			...
			System.out.println("[NaverCal] Past " + days + " event query in progress...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getThisWeek() {
			...
			System.out.println("[NaverCal] This week event query in progress...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getThisMonth() {
			...
			System.out.println("[NaverCal] This month event query in progress...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getNextMonth() {
			...
			ZonedDateTime start = firstDay.atStartOfDay(ZoneId.systemDefault());
			ZonedDateTime end   = lastDay.atTime(23, 59, 59).atZone(ZoneId.systemDefault());
			System.out.println("[NaverCal] Next month event query in progress...");
			return getEvents(start, end);
		}
		...
		public List<CalendarEvent> getUpcomingAlarms(int withinMinutes) {
			...
			System.out.println("[NaverCal] " + withinMinutes + "minutes within event query in progress...");
		...
		private List<CalendarEvent> getEvents(ZonedDateTime start, ZonedDateTime end) {
			...
			if (!initialized) {
				System.out.println("[NaverCal WARNING] service not initialized");
				return result;
			}
			processedEventKeys.clear(); // 조회마다 initialized (누적 중복 방지)
			
			try {
				List<String> calUrls = fetchCalendarUrls(calendarHomeUrl);
				if (calUrls.isEmpty()) {
					System.out.println("[NaverCal WARNING] calendar URL missing");
					return result;
				}
				System.out.println("[NaverCal] calendar " + calUrls.size() + " items found");
				
								for (String calUrl : calUrls) {
					if (result.size() >= MAX_EVENTS_PER_REQUEST) {
						System.out.println("[NaverCal WARNING] maximum event count reached: " + MAX_EVENTS_PER_REQUEST);
						break;
					}
					try {
						...
						for (String icsText : icsTexts) {
							...
							try {
								...
								System.out.println("[NaverCal ERROR] ICS parsing error: " + e.getMessage());
							}
						}
					} catch (Exception e) {
						System.out.println("[NaverCal ERROR] calendar processing error: " + e.getMessage());
					}
				}
								result.sort((a, b) -> a.startTime.compareTo(b.startTime));
				System.out.println("[NaverCal] total " + result.size() + " items events found");
				
				} catch (Exception e) {
				System.out.println("[NaverCal ERROR] error while querying events: " + e.getMessage());
				e.printStackTrace();
			}
			return result;
		}
		...
		private List<CalendarEvent> parseIcsWithIcal4j(String icsText,
			...
			try {
				...
				Period<LocalDate> datePeriod = new Period<>(
					qStart.toLocalDate(), qEnd.toLocalDate().plusDays(1)); // all-day 이벤트용
				...
				for (Object comp : cal.getComponents(Component.VEVENT)) {
					...
					try {
						...
						String title = (sumProp != null) ? sumProp.getValue() : "(No title)";

						// DTSTART VALUE=DATE 이면 종, 없거나 DATE-TIME 이면 시간 기반
						...
						if (msg.contains("Unsupported unit") || msg.contains("Unable to obtain")) {
							System.out.println("[NaverCal DEBUG] sub-event parsing Failed (ignored): " + msg);
						} else {
							System.out.println("[NaverCal ERROR] VEVENT processing error: " + msg);
						}
					}
				}
			} catch (Exception e) {
		 ...
		 * DTSTART/RRULE/EXDATE 를 직접 File싱Other여 ZonedDateTime 으로 전개한다.
		 *
		 * 처리 순서:
		 *  1) DTSTART 를 ZonedDateTime 으로 File싱 (TZID / Z / floating)
		...
		private List<ZonedDateTime[]> expandTimed(VEvent vevent,
			...
			try {
				...
				// 반복 전개 상한: 조회 종료 또는 UNTIL 중 이른 것
				...
				while (!cur.isAfter(expandEnd) && generated < count) {
					...
					switch (freq) {
						...
						default: return result; // 알 Wed 없는 FREQ
					}
				}
			} catch (Exception e) {
				System.out.println("[NaverCal ERROR] expandTimed error: " + e.getMessage());
			}
			return result;
		}
		...
		private ZonedDateTime extractZonedDateTime(VEvent vevent, String propName,
			...
			try {
				...
				System.out.println("[NaverCal ERROR] extractZonedDateTime error: " + e.getMessage());
				return occurrenceDate.atStartOfDay(ZoneId.systemDefault());
			}
		}
		...
		private List<String> extractIcsHrefs(String xmlResp) {
			...
			try {
				...
				System.out.println("[NaverCal ERROR] ICS href extract error: " + e.getMessage());
			}
			return hrefs;
		}
		...
		private List<String> fetchCalendarUrls(String homeUrl) {
			...
			try {
				...
				System.out.println("[NaverCal ERROR] calendar URL query error: " + e.getMessage());
			}
			return urls;
		}
		 ...
		 *
		 * 1단계: PROPFIND(depth=1) → All .ics href 목록 + etag 수신 (빠름)
		 * 2단계: calendar-multiget REPORT → href 목록을 서버에 보내 ICS 본문 괄 수신 (1회)
		 *
		 * 네이버 CalDAV는 time-range 필터(calendar-query)를 무시Other므로
		 * multiget 으로 All ICS 를 한 번에 받은 뒤 클라이언트에서 날짜 필터링한다.
		 */
		private List<String> fetchIcsTexts(String calUrl,
			...
			System.out.println("[NaverCal] " + calName + " → .ics " + hrefs.size() + " items");
			if (hrefs.isEmpty()) return new ArrayList<>();

			// ── 2단계: calendar-multiget REPORT → ICS 본문 괄 수신 ─
			try {
				...
				if (multigetResp != null && !multigetResp.trim().isEmpty()) {
					...
					if (!icsTexts.isEmpty()) {
						System.out.println("[NaverCal] multiget success → " + calName + " " + icsTexts.size() + " items");
						return icsTexts;
					}
				}
				System.out.println("[NaverCal] multiget no response → individual GET fallback");
			} catch (Exception e) {
				System.out.println("[NaverCal] multiget Failed → individual GET fallback: " + e.getMessage());
			}
			...
			for (String href : hrefs) {
				try {
					...
					System.out.println("[NaverCal ERROR] GET Failed: " + e.getMessage());
				}
			}
			return result;
		}
		 ...
		 * href 목록을 서버에 전달Other면 해당 ICS 본문을 한 번에 반환한다.
		 * time-range 필터 없이 href 기반으로 동작Other므로 네이버도 지원한다.
		 */
		private String calendarMultigetBody(List<String> hrefs) {
			...
			sb.append("  </D:prop>\n");
			for (String href : hrefs) {
				// href가 절대Path(/caldav/...)면 그대로, 상대Path면 그대로 Enable
		 ...
		 * XML File서로 calendar-data 태그 내용을 수집한다.
		 */
		private List<String> extractCalendarData(String xmlResp) {
			...
			try {
				...
				db.setErrorHandler(null);
				Document doc = db.parse(new InputSource(new StringReader(xmlResp)));
				// calendar-data 는 네임스페이스 무관Other게 로컬명으로 검색
				...
				System.out.println("[NaverCal ERROR] calendar-data extract error: " + e.getMessage());
			}
			return list;
		}
		...
		private String caldavRequest(String method, String urlStr,
			...
			*/
			...
			try (javax.net.ssl.SSLSocket sock =
				...
				if (statusCode == 401) {
					System.out.println("[NaverCal ERROR] authentication failed (401) - check ID/password");
					return null;
				}
				if (statusCode == 0 || statusCode >= 500) {
					System.out.println("[NaverCal ERROR] HTTP error: " + statusCode);
					return null;
				}
		...
		private String dechunk(String chunked) {
			...
			while (pos < chunked.length()) {
				...
				try {
					...
					System.out.println("[NaverCal ERROR] dechunk error: " + e.getMessage());
					break;
				}
				if (size == 0) break;
				pos = lineEnd + 2;
		...
		public void cleanup() {
			...
			System.out.println("[NaverCal] resource cleanup done");
		}
