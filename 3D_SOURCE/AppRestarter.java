...
public class AppRestarter {
	...
	*/
    ...
    public void restartApp() {
        ...
        System.out.println("[Restart] restart request"
	...
	*/
    public void buildAppCdsIfNeeded(Runnable saveConfig) {
        ...
        if (new java.io.File(jsaPath).exists()) {
            if (!jsaPath.equals(cachedJsaPath)) {
                ...
                System.out.println("[AppCDS] existing JSA enabled: " + jsaPath);
			}
            return;
		}
        ...
        if (javaw.toLowerCase().contains("runtime" + java.io.File.separator + "bin")) {
            ...
            if (sysJavaw != null) {
                System.out.println("[AppCDS] jpackage javaw detected → system javaw Enable: " + sysJavaw);
                javaw = sysJavaw;
				} else {
                System.out.println("[AppCDS] system javaw search failed - skipping JSA generation");
                return;
			}
		}
        ...
        new Thread(() -> {
            try {
                System.out.println("[AppCDS] JSA creation Start: " + fJsa);
                ...
                if (new java.io.File(fJsa).exists()) {
                    cachedJsaPath = fJsa;
                    if (saveConfig != null) saveConfig.run();
                    System.out.println("[AppCDS] JSA creation done: " + fJsa);
					} else {
                    System.out.println("[AppCDS] JSA creation failed (File not found)");
				}
				} catch (Exception e) {
                System.out.println("[AppCDS] JSA creation error: " + e.getMessage());
			}
    ...
    private Runnable buildRestartRunnable(String tgMsg) {
        return () -> {
			/*
				...
				tgThread.start();
				tgThread.join(10000);
				System.out.println("[Restart] Telegram send done");
                } catch (Exception e) {
				System.out.println("[Restart] Telegram send Failed: " + e.getMessage());
                }
				}
			*/
            try {
                ...
                if (exePath.isEmpty() || !new java.io.File(exePath).exists()) {
                    System.out.println("[Restart] Path cache missing - search start");
                    exePath = resolveExePathForRestart();
					} else {
                    System.out.println("[Restart] cached Path Enable: " + exePath);
				}
                if (exePath == null) {
                    System.out.println("[Restart] executable file Path search Failed , Restart Failed");
                    return;
				}
                        ...
                        if (exeFiles != null) {
                            for (java.io.File ef : exeFiles) {
                                ...
                                if (n.contains("kootpanking")) {
                                    ...
                                    System.out.println("[Restart] jar→exe switch: " + exePath);
                                    break outer;
								}
							}
                            if (exeFiles.length > 0) {
                                exePath = exeFiles[0].getAbsolutePath();
                                System.out.println("[Restart] jar→exe switch(first): " + exePath);
                                break;
							}
						}
                ...
                if (exePath.endsWith(".exe")) {
                    ...
                    if (!javawExists) {
                        ...
                        System.out.println("[Restart] javaw Path search: " + javaw);
						} else {
                        System.out.println("[Restart] cached javaw Enable: " + javaw);
					}
                ...
                System.out.println("[Restart] INI cache exePath=" + cachedExePath
				+ (cachedJavawPath.isEmpty() ? "" : " javawPath=" + cachedJavawPath));
                pb.directory(new java.io.File(exePath).getParentFile());
                pb.start();
                System.out.println("[Restart] new process start Done: " + exePath);
                System.exit(0);
				} catch (Exception ex) {
                System.out.println("[Restart] restart failed: " + ex.getMessage());
			}
		};
	}
    // ── 내부: Path 탐색 ─────────────────────────────────────
        ...
        for (int up = 0; up < 4; up++) {
            ...
            if (exeFiles != null && exeFiles.length >= 1) {
                for (java.io.File ef : exeFiles) {
                    String n = ef.getName().toLowerCase();
                    if (n.contains("kootpanking")) {
                        System.out.println("[Restart] exe search(name match): " + ef.getAbsolutePath());
                        return ef.getAbsolutePath();
					}
				}
                System.out.println("[Restart] exe search(first): " + exeFiles[0].getAbsolutePath());
        ...
        public static boolean set(boolean enable) {
            try {
                ...
                if (enable) {
                    ...
                    System.out.println("[AutoStart] registered: " + cmdValue);
                ...
                System.out.println("[AutoStart] error: " + e.getMessage());
                return false;
			}
		}
        /**
		...
		*/
        private static String buildCmdValue() {
            ...
            if (exePath != null
                ...
                System.out.println("[AutoStart] exe mode registered: " + exePath);
                return exePath;
			}
            ...
            if (jarPath != null && jarPath.endsWith(".jar")) {
                System.out.println("[AutoStart] jar mode registered: " + javaw + " -jar " + jarPath);
    ...
    public static class ShutdownGuard {
        ...
        public synchronized void register() {
            ...
            hookThread = new Thread(() -> {
                if (cancelled) {
                    System.out.println("[ShutdownGuard] normal shutdown — skip notice");
                    return;
				}
                System.out.println("[ShutdownGuard] shutdown signal detected — notice send start");
                sendNotifications();
                System.out.println("[ShutdownGuard] Done");
                // AppLogger.close();
			}, "ShutdownGuard-Hook");
            Runtime.getRuntime().addShutdownHook(hookThread);
            System.out.println("[ShutdownGuard] registration done");
		}
        /** 정상 Shutdown h 호출 — Notice을 보내지 않는다. */
        public void cancel()  { cancelled = true;  System.out.println("[ShutdownGuard] Notice canceled (normal shutdown)"); }
        /** cancel() 을 되돌린다. */
        ...
        private void sendTelegramExit() {
			...
			try {
                tg.sendTelegramExit();
                System.out.println("[ShutdownGuard] Telegram send done");
				} catch (Exception e) {
                System.out.println("[ShutdownGuard] Telegram send failed: " + e.getMessage());
			}
		}
        private void sendEmail(String now, String pcName, String userId) {
            ...
            try {
                ...
                System.out.println("[ShutdownGuard] email send done");
				} catch (Exception e) {
                System.out.println("[ShutdownGuard] email send failed: " + e.getMessage());
			}
		}
		...
		*/
        ...
        private static void ensureYtDlp(String toolsDir) throws Exception {
            ...
            if (java.nio.file.Files.exists(exe)) {
                System.out.println("[ToolManager] yt-dlp already exists");
                return;
			}
        ...
        private static void ensureFfmpeg(String toolsDir) throws Exception {
            ...
            if (java.nio.file.Files.exists(exe)) {
                System.out.println("[ToolManager] ffmpeg already exists");
                return;
			}
            System.out.println("[ToolManager] downloading ffmpeg... (100MB+, takes time)");
            ...
            try {
                ...
                System.out.println("[ToolManager] extracting ffmpeg archive...");
				...
				.ifPresent(found -> {
					try {
						...
						System.out.println("[ToolManager] ffmpeg.exe copy done: " + found);
				...
				.forEach(dir -> {
					try {
						...
						System.out.println("[ToolManager] temporary folder delete: " + dir);
					} catch (java.io.IOException e) { e.printStackTrace(); }
				});
	...
	public static void doUpgrade() {
		...
		System.out.println("Install folder: " + defaultInstallDir.getAbsoluteFile());
		...
		new Thread(() -> {
			try {
				...
				if (AppRestarter.AutoStart.check()) {
					...
					System.out.println(unregistered	? "✅ startup program unregister done"
					: "⚠️  Auto-start unregister failed (ignored, continuing)");
					} else {
					System.out.println("ℹ️  startup program not registered — skip unregister");	
				}
				...
				// 3. zip 압축 해제 ──────────────────────────────────────────
				System.out.println("[Upgrade] archive extraction start");
                ...
                while ((entry = zis.getNextEntry()) != null) {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        System.out.println("[Upgrade] extract: " + dest.getAbsolutePath());
					}
                    zis.closeEntry();
				}
				}
				System.out.println("[Upgrade] archive extraction done");
				...
				System.out.println("[Upgrade] execution target: " + runTarget.getAbsolutePath());
				...
				Platform.runLater(() -> {
					System.out.println("🚀 Updater launched — the program will now exit.");	
				...
				System.out.println("❌ upgrade error: " + ex.getMessage());				
