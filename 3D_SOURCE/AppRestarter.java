...
public class AppRestarter {
    ...
    /** TelegramBot 인스턴스를 나중에 연결/교체한다. (main 선생성 후 앱 초기화 시 주입용) */
	...
	*/
    public void buildAppCdsIfNeeded(Runnable saveConfig) {
        ...
        if (new java.io.File(jsaPath).exists()) {
            if (!jsaPath.equals(cachedJsaPath)) {
                ...
                System.out.println("[AppCDS] 기존 JSA 사용: " + jsaPath);
			}
            return;
		}
        ...
        if (javaw.toLowerCase().contains("runtime" + java.io.File.separator + "bin")) {
            ...
            if (sysJavaw != null) {
                System.out.println("[AppCDS] jpackage javaw 감지 → 시스템 javaw 사용: " + sysJavaw);
                javaw = sysJavaw;
				} else {
                System.out.println("[AppCDS] 시스템 javaw 탐색 실패 - JSA 생성 스킵");
                return;
			}
		}
        ...
        new Thread(() -> {
            try {
                System.out.println("[AppCDS] JSA 생성 시작: " + fJsa);
                ...
                if (new java.io.File(fJsa).exists()) {
                    cachedJsaPath = fJsa;
                    if (saveConfig != null) saveConfig.run();
                    System.out.println("[AppCDS] JSA 생성 완료: " + fJsa);
					} else {
                    System.out.println("[AppCDS] JSA 생성 실패 (파일 없음)");
				}
				} catch (Exception e) {
    ...
    private Runnable buildRestartRunnable(String tgMsg) {
        return () -> {
			/*
				...
				tgThread.start();
				tgThread.join(10000);
				System.out.println("[Restart] 텔레그램 전송 완료");
                } catch (Exception e) {
				System.out.println("[Restart] 텔레그램 전송 실패: " + e.getMessage());
                }
				}
			*/
            try {
                ...
                if (exePath.isEmpty() || !new java.io.File(exePath).exists()) {
                    System.out.println("[Restart] 경로 캐시 없음 - 탐색 시작");
                    exePath = resolveExePathForRestart();
					} else {
                    System.out.println("[Restart] 캐시된 경로 사용: " + exePath);
				}
                if (exePath == null) {
                    System.out.println("[Restart] 실행 파일 경로 탐색 실패 , Restart 실패");
                    return;
				}
                ...
                if (exePath.endsWith(".exe")) {
                    ...
                    if (!javawExists) {
                        ...
                        System.out.println("[Restart] javaw 경로 탐색: " + javaw);
						} else {
                        System.out.println("[Restart] 캐시된 javaw 사용: " + javaw);
					}
                    ...
                    if (!cachedJsaPath.isEmpty() && new java.io.File(cachedJsaPath).exists()) {
						...
						"-jar", exePath);
                        System.out.println("[Restart] AppCDS JSA 적용: " + cachedJsaPath);
					}
				}
                System.out.println("[Restart] INI 캐시 exePath=" + cachedExePath
				+ (cachedJavawPath.isEmpty() ? "" : " javawPath=" + cachedJavawPath));
                pb.directory(new java.io.File(exePath).getParentFile());
                pb.start();
                System.out.println("[Restart] 새 프로세스 시작 완료: " + exePath);
                System.exit(0);
				} catch (Exception ex) {
                System.out.println("[Restart] 재시작 실패: " + ex.getMessage());
			}
		};
	}
    // ── 내부: 경로 탐색 ─────────────────────────────────────
    ...
    public static class ShutdownGuard {
        ...
        /** Shutdown Hook 등록. 앱 전체에서 1회만 호출한다. */
        public synchronized void register() {
            if (hookThread != null) return;
            hookThread = new Thread(() -> {
                if (cancelled) {
                    System.out.println("[ShutdownGuard] 정상 종료 — 알림 생략");
                    return;
				}
	...
	public static class PCShortcut {
		...
		public interface FavoriteCallback {
			/** name: lnk 파일명(확장자 제외), path: 타겟 full path */
		...
		public javafx.scene.control.Menu createMenuWithWindow(javafx.stage.Stage owner) {
			...
			new javafx.scene.control.MenuItem("🪟 앱 창으로 열기");
			openWin.setOnAction(e -> showWindow(owner));
			menu.getItems().add(0, openWin);
			return menu;
		}
		public javafx.scene.control.Menu createMenu() {
			javafx.scene.control.Menu rootMenu = new javafx.scene.control.Menu("바탕화면");
			javafx.scene.control.Menu normalAppsMenu = new javafx.scene.control.Menu("일반 앱");
			javafx.scene.control.Menu windowsAppsMenu = new javafx.scene.control.Menu("Windows 앱");
			rootMenu.getItems().add(normalAppsMenu);
			rootMenu.getItems().add(windowsAppsMenu);
			javafx.scene.control.MenuItem refreshNormal =
            new javafx.scene.control.MenuItem("새로고침");
		...
		private void refreshNormalAppsMenu(javafx.scene.control.Menu normalAppsMenu) {
			...
			if (entries.isEmpty()) {
                ...
                new javafx.scene.control.MenuItem("(없음)");
		...
		private java.util.List<AppEntry> collectGeneralApps___() {
			...
			lnkFiles.addAll(listLnkFilesRecursive(commonPrograms));
			// 4) C:\Users\사용자명\AppData\Roaming\Microsoft\Windows\Start Menu 의 모든 lnk
			lnkFiles.addAll(listLnkFilesShallow(userStartMenu));
			// 5) C:\Users\사용자명\AppData\Roaming\Microsoft\Windows\Start Menu\Programs 의 모든 lnk
			lnkFiles.addAll(listLnkFilesShallow(userPrograms));
			// 6) C:\Users\사용자명\AppData\Roaming\Microsoft\Windows\Start Menu\Programs 재귀
			lnkFiles.addAll(listLnkFilesRecursive(userPrograms));
			// 7) 파일명.lnk 기준 sort
			lnkFiles.sort(java.util.Comparator.comparing(
				f -> f.getName().toLowerCase(java.util.Locale.ROOT),
		...
		private java.util.List<AppEntry> collectWindowsApps() {
			...
			// AppEntry 생성: parseLnkTarget 으로 실제 exe 경로 추출
			java.util.List<AppEntry> result = new java.util.ArrayList<>();
			for (File f : dedup.values()) {
				String lnkTarget = parseLnkTarget(f);
				String execTarget = (lnkTarget != null && !lnkTarget.isEmpty())
		...
		// ─────────────────────────────────────────────
		// [ MainWindow_Menu / 바탕화면 / Windows 앱 / 제어판 ]
		// ─────────────────────────────────────────────
		...
		private String getLetterGroup(String name) {
			if (name == null || name.isBlank()) return "기타";
		...
		private String parseLnkTarget(java.io.File lnkFile) {
			...
			try {
				...
				// ② LinkInfo → LocalBasePath 시도 (성공 시 즉시 반환)
				if (hasLinkInfo && pos + 28 <= d.length) {
					int liStart   = pos;
					int liSize    = u32le(d, liStart);       // 구조체 전체 크기
				...
				if (iconLoc != null && !iconLoc.isEmpty()) {
					...
					if (!c.isEmpty()) return c; // 파일 없어도 경로 반환
				}
				...
				if (workDir != null && !workDir.isEmpty()) {
					...
					return workDir; // 최소한 디렉토리라도 반환
				}
			} catch (Exception ignored) {}
			return "";
		}
		/** 환경변수 확장: %SystemRoot% → C:\Windows 등 */
