...
/**
 * ItsCctvManager (JavaFX Version)
 *
 * ── API ──────────────────────────────────────────────────────
 *   엔드포인트 : https://openapi.its.go.kr:9443/cctvInfo
 *   File라미터   : apiKey, type=its(국도), cctvType=3(Stopped영상 JPEG URL),
 ...
 *   JavaFX Timeline 으로 REFRESH_SEC 초마다 Image를 fetch →
...
public class ItsCctvManager {
    ...
    private static final int REFRESH_SEC      = 5;        // Stopped영상 갱신 주기 (s)
    ...
    public static class CctvItem {
        ...
        public final String url;   // Stopped영상 JPEG URL
    ...
    public interface HostCallback {
        /**
         * 갱신된 JPEG Image를 배경으로 Settings.
         * FX Application Thread 에서 호출됨이 보장된다.
         *
         * @param label  "[ITS CCTV] 카메라명" 형태 레이블 (로그/UI 표시용)
         * @param image  변환된 WritableImage. null 이면 배경 initialization.
         */
        void setItsCctvImage(String label, WritableImage image);

        /** Theme 갱신 요청. FX Application Thread 에서 호출됨이 보장된다. */
        void repaintClock();
    }
    ...
    private List<CctvItem>   activeItems = new ArrayList<>();  // Filter Apply list (없으면 All)
     ...
     * Done h FX 스레드에서 onSuccess / onError 콜백.
     */
    public void fetchList(Runnable onSuccess, Consumer<String> onError) {
        new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    items       = result;
                    activeItems = new ArrayList<>(result);  // 기본: All
                    current     = 0;
                    onSuccess.run();
                });
            } catch (Exception e) {
     ...
     * Select 다이얼로그에서 Select한 아이템의 items(All) 인덱스를 받아
    ...
    public void select(int globalIndex) {
        ...
        if (ai < 0) {
            // 필터에 없는 항목 → activeItems 를 All로 리셋
    ...
    /**
     * 필터 Settings (Select 다이얼로그의 필터 필드 확인 h 호출).
     * keyword 가 비어 있으면 All 목록으로 복원.
    ...
    public void setFilter(String keyword) {
        ...
        current = 0;
        System.out.println("[ItsCctv] Filter: " + keyword + " -> " + activeItems.size() + " items");
    }
    ...
    public void start() {
        ...
        running = true;
        stopTimeline();
        refreshNow();          // 즉h 1회 Show
        startTimeline();
        System.out.println("[ItsCctv] Start: " + currentName());
    }

    /** 갱신 타이머 Stop — FX 스레드에서 호출해야 한다. */
    public void stop() {
        running = false;
        refreshPending = false;
        refreshVersion.incrementAndGet();
        stopTimeline();
        System.out.println("[ItsCctv] Stopped");
    }
    ...
    private void fetchAndDisplay(CctvItem item, long version) {
        ...
        refreshExecutor.execute(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    if (!running) return;   // Stop 후 큐 잔류 차단
                ...
                if (version == refreshVersion.get()) {
                    System.out.println("[ItsCctv] Image update failed: " + e.getMessage());
                }
            } finally {
     ...
     * ARGB 픽셀 배열을 통해 변환Other므로 javax.imageio 외 별도 라이브러리 불필요.
     */
    private static WritableImage awtToFx(BufferedImage src) {
        int w = src.getWidth();
        int h = src.getHeight();

        // TYPE_INT_ARGB 로 통 (원본 타입이 달라도 안전Other게 변환)
        BufferedImage argb;
    ...
    private List<CctvItem> callApi() throws Exception {
        if (apiKey.isEmpty()) throw new Exception("API key not entered.");
    ...
    /** JSON File싱 (외부 라이브러리 없이 substring 방식) */
    private List<CctvItem> parseJson(String json) throws Exception {
        ...
        if (dataIdx < 0) {
            ...
            if (msg.isEmpty()) msg = "No data (no CCTV in range)";
            throw new Exception(msg);
        }

        int arrStart = json.indexOf('[', dataIdx);
        int arrEnd   = findMatchingBracket(json, arrStart);
        if (arrStart < 0 || arrEnd < 0) throw new Exception("JSON parse error");
        ...
        if (list.isEmpty()) throw new Exception("No active CCTV in range");
        return list;
    }
