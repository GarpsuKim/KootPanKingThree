...
public class GmailSender {
	...
	*/
	...
	public String testSend(String from, String pass, String to, List<File> attachs	)  {
		if (from == null || from.trim().isEmpty()) {
			System.out.println("[Gmail][testSend] Failed [Sender ID not set]");
			return "[Gmail][testSend] Failed [Sender ID not set]";
		}
		if (pass == null || pass.trim().isEmpty()) {
			System.out.println("[Gmail][testSend] Failed [Sender app password not set]");
			return "[Gmail][testSend] Failed [Sender App Password not set]";
		}
		if (to == null || to.trim().isEmpty()) {
			System.out.println("[Gmail][testSend] Failed [Recipient ID not set]");
	...
	*/
    ...
    public void sendStartupNotice(String textContent) {
        if (!isConfigured() || lastTo.isEmpty()) {
            System.out.println("[Gmail][sendStartupNotice] skip — from=" + from
	...
	*/
    public void sendShutdownNotice(Runnable afterSend) {
        if (!isConfigured() || lastTo.isEmpty()) {
            System.out.println("[Gmail][sendShutdownNotice] skip — from=" + from
			+ " lastTo=" + lastTo + " configured=" + isConfigured());
	...
	*/
    public void sendShutdownNoticeSync() {
        if (!isConfigured() || lastTo.isEmpty()) {
            System.out.println("[Gmail][sendShutdownNoticeSync] skip — from=" + from
			+ " lastTo=" + lastTo + " configured=" + isConfigured());
            return;
		}
        try {
	...
	*/
	void smtpSend(String user, String pass,
        ...
        System.out.println("[Gmail][SMTP] connecting → " + SMTP_HOST + ":" + SMTP_PORT);
        ...
        System.out.println("[Gmail][SMTP] 220 server greeting OK");
        ...
        System.out.println("[Gmail][SMTP] AUTH LOGIN authentication successful");
        // 4) Sending email
        ...
        System.out.println("[Gmail][SMTP] DATA start");
        ...
        System.out.println("[Gmail][SMTP] send done 250 OK → " + to);
        swr.println("QUIT");
        ssl.close();
        sock.close();
	}
	...
	*/
	...
	public String sendOneSmtpWithAttachment(
		...
		if (body == null) body = "";
		try {
			System.out.println("[Gmail][attach] connecting → " + SMTP_HOST + ":" + SMTP_PORT);
